...
Check the chains to see the share of null decisions and self-cancelling actions.
```{r}
### value meanings
...
for (d in 1:D){
  theChain <- saomResults_K2[[d]]$chain #for each imputation
...
for (r in 1:length(theChain)){ #for each run
  for (p in 1:length(theChain[[r]][[1]])){ #for each period (there is only one group)
    for (m in 1:length(theChain[[r]][[1]][[p]])){ #for each ministep
      ...
      chains_K2_list[[num]] <- c(imp,run, period, ministeps.period, ministep, ego, alter)
      num <- num+1
    }
  }
}
...
chains_K2_df <-as.data.frame(do.call(rbind, chains_K2_list))
colnames(chains_K2_df) <- c("imp", "run", "period", "ministeps.period", "ministep", "ego", "alter")
```


```{r}
chains_K2_dfx <- chains_K2_df %>% 
...
```{r}
#save chain results
stargazer(chains_K2_dfx, summary = FALSE, title=paste0("Null and self-cancelling mini-steps K2"),
          rownames = TRUE, type = "html", out = "Results/SAOM_chain_K2.html")
```
...
#create a dataframe with all estimated parameters and standard errors
...
stargazer(finalResults_rounded_K2, summary = FALSE, title=paste0("Analysis results cohort 2, 1st wave imputed with stationary SAOM ",D,"imputations"),
          rownames = TRUE, type = "html", out = "Results/SAOM_analysis_results_K2_chains.html")

```

```{r}
todaysdate <- format(Sys.Date(),"%y%m%d")
save(list = ls(.GlobalEnv), file = paste0(todaysdate,"_analysis_cohort2_environment_",D,"imp_chains.Rdata"))
```
